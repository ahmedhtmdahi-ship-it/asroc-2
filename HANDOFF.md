...
- أول إقلاع بيطبّق الـ migrations ويزرع البيانات تلقائيًا (idempotent): 35 قسم + 19343 دواء
  + المستخدمين. **صورة Docker خالية من الـ PII** — افتراضيًا بتزرع داتا صناعية (`users.json`).

**زرع الداتا الحقيقية (on-prem):** الملف `users.local.json` مستبعد من git ومن الصورة عمدًا.
لزرع الـ 1755 موظف الحقيقيين: انسخ الملف للسيرفر، وشيل التعليق عن الـ mount و`SEED_USERS_FILE`
في `docker-compose.yml` (بيركّبه read-only وقت التشغيل بدل حقنه في الصورة).
