...
@pytest.fixture
def seeded(db):
    """One project, full roster, one past session with a fake sheet."""
    prog = Project(name="React", attendance_rules={}, attendance_sheet_id="sheet-1",
                   attendance_sheet_tab="Attendance", feedback_form_url="https://forms.example/x")
    db.add(prog)
    db.flush()
    trainees = [Trainee(project_id=prog.id, name_ar=ar, name_en=en, email=em) for ar, en, em in ROSTER]
    db.add_all(trainees)
    start = datetime(2026, 8, 17, 16, 0)     # 18:00 Cairo
    sess = Sess(project_id=prog.id, title="React", planned_start=start,
    ...
    return {"project": prog, "session": sess, "trainees": trainees, "start": start}
