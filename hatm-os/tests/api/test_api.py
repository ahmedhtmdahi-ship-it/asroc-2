...
def test_setup_endpoints(client):
    p = client.post("/api/projects", json={"name": "Flutter"}).json()
    t = client.post("/api/trainees", json={"project_id": p["id"], "name_ar": "منى سمير",
                                           "email": "mona@example.com"}).json()
    s = client.post("/api/sessions", json={"project_id": p["id"], "title": "Flutter",
                                           "planned_start": "2026-09-01T18:00:00+03:00",
                                           "planned_end": "2026-09-01T20:00:00+03:00"}).json()
    assert t["id"] and s["status"] == "scheduled"
    assert client.get(f"/api/projects/{p['id']}/trainees").json()["trainees"][0]["name_ar"] == "منى سمير"
