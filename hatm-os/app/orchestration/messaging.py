...
def build_variables(db, t: Trainee, sess: Sess | None, prog: Project, rec=None) -> dict:
    ...
    if sess:
        ...
        nxt = db.query(Sess).filter(Sess.project_id == prog.id,
...
def compose(db, *, trainee: Trainee, msg_type: str, session: Sess | None, project: Project,
    ...
    r = render(tpl["body"], build_variables(db, trainee, session, project, rec), tpl["subject"])
    m = OutboundMessage(
    ...
    try:
        ...
        for m in rows:
            ...
            if not m.to_address:
                ...
                skipped += 1
                continue
            ok, why = (AntiSpamGate.check(person_id=m.person_id, db=db) if m.person_id
                       else AntiSpamGate.check(m.trainee_id, db))
            if not ok:
            ...
            record_activity("message_sent" if m.status == "sent" else f"message_{m.status}",
                            target_type="person" if m.person_id else "trainee",
                            target_id=m.person_id or m.trainee_id,
