...
def s3_match_names(ctx: dict) -> dict:
    ...
    try:
        ...
        roster = db.query(Trainee).filter_by(project_id=sess.project_id, status="active").all()
...
def s4_evaluate(ctx: dict) -> dict:
    ...
    try:
        sess = db.get(Sess, ctx["session_id"])
        _set_status(db, sess, "running")
        rules = sess.project.attendance_rules or {}
        ...
        for t in db.query(Trainee).filter_by(project_id=sess.project_id, status="active").all():
...
def s7_compose_messages(ctx: dict) -> dict:
    ...
    try:
        ...
        for r in recs:
            ...
            m = compose(db, trainee=t, msg_type=msg_type, session=sess, project=prog, rec=r)
            ids.append(m.id)
        db.commit()
