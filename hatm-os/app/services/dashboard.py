...
def collect_priority_items(db) -> list[PriorityItem]:
    ...
    for s in db.query(Sess).filter(Sess.status == "scheduled", Sess.planned_end < now).all():
        n = db.query(func.count(Trainee.id)).filter_by(project_id=s.project_id, status="active").scalar()
        items.append(PriorityItem(
    ...
    try:
        ...
        return {
            ...
            "projects": [{"id": p.id, "name": p.name} for p in
                         db.query(Project).filter_by(status="active").all()],
        }
    finally:
        db.close()


def charts(project_id: int | None = None, limit: int = 12) -> dict:
    db = SessionLocal()
    try:
