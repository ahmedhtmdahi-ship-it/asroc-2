...
MESSAGE_STATUSES = ("draft", "pending_approval", "approved", "sent", "failed",
                    "skipped", "excluded", "drafted")

# telegram → the team channel · email → trainees · draft → rendered only, never sent
# (draft is how group announcements work: the system writes the text, Hatem pastes it)
CHANNELS = ("telegram", "email", "draft")
...
class OutboundMessage(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    # exactly one recipient side is set: a team member (task follow-up) or a
    # trainee (attendance follow-up)
    person_id: Mapped[int | None] = mapped_column(ForeignKey("people.id"), index=True)
    trainee_id: Mapped[int | None] = mapped_column(ForeignKey("trainees.id"), index=True)
