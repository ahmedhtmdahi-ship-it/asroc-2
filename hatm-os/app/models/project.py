...
class Project(Base):
    """A programme or event: أسيوط بشبابها · برنامج React · برنامج Flutter."""
    ...
    # root of the project's folder tree; task folders are indexed under it
    drive_root_folder_id: Mapped[str | None] = mapped_column(String(100))
    # the sheet holding this project's task list (phase 1)
    tasks_sheet_id: Mapped[str | None] = mapped_column(String(100))
    tasks_sheet_tab: Mapped[str | None] = mapped_column(String(100))
    ...
    trainees: Mapped[list["Trainee"]] = relationship(back_populates="project")  # noqa: F821
