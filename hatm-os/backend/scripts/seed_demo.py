"""Seed a demo program (faked Arabic names) + a past session so the flow can be driven end-to-end.
...
def run() -> None:
    db = SessionLocal()
    try:
        if db.query(Program).filter_by(name="React").first():
            print("already seeded")
            return
        prog = Program(name="React", attendance_rules={}, attendance_sheet_id="demo-sheet",
        ...
        db.add(prog)
        db.flush()
        db.add_all([Trainee(program_id=prog.id, name_ar=ar, name_en=en, email=em)
                    for ar, en, em in ROSTER])
        yesterday = (utcnow() - timedelta(days=1)).replace(hour=15, minute=0, second=0, microsecond=0)
        db.add(Sess(program_id=prog.id, title="React", planned_start=yesterday,
        ...
        db.add(Sess(program_id=prog.id, title="React", planned_start=tomorrow,
                    planned_end=tomorrow + timedelta(hours=2), zoom_meeting_id="81244710932"))
        flutter = Program(name="Flutter", attendance_rules={})
        db.add(flutter)
        db.flush()
        today = utcnow().replace(hour=15, minute=0, second=0, microsecond=0)
        db.add(Sess(program_id=flutter.id, title="Flutter", planned_start=today,
        ...
        print(f"seeded program {prog.id} with {len(ROSTER)} trainees and 3 sessions")
    finally:
        db.close()
