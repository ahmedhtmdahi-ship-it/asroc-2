...
def pending_feedback(db, program_id: int | None = None) -> list[OutboundMessage]:
    ...
    for m in rows:
        if program_id is not None:
            ...
            if not t or t.program_id != program_id:
                continue
...
def create_reminder_approval(program_id: int | None = None) -> dict:
    """Compose reminders for unanswered feedback and put them behind ONE approval."""
    db = SessionLocal()
    try:
        targets = pending_feedback(db, program_id)
        if not targets:
            return {"created": 0}
        msgs = []
        for fb in targets:
            ...
            prog = db.get(Program, t.program_id)
            ...
            m = compose(db, trainee=t, msg_type="reminder", session=sess, program=prog)
        ...
        a = ApprovalGate().create(
            type="send_messages",
            payload={"stage": "reminders", "program_id": program_id,
