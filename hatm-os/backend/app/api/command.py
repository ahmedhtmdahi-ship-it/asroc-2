...
def _context(db) -> dict:
    progs = db.query(Program).filter_by(status="active").all()
    ...
    return {"active_programs": [{"id": p.id, "name": p.name} for p in progs],
            "recent_sessions": [{"id": s.id, "program_id": s.program_id,
...
def command(body: CommandIn, actor: str = Depends(current_user), db: DBSession = Depends(get_db)):
    ...
    if entities.get("program_id") not in valid_programs:
        entities["program_id"] = None

    if intent == "close_session":
        sid = entities.get("session_id")
        if not sid:
            cands = [s for s in ctx["recent_sessions"]
                     if not entities.get("program_id") or s["program_id"] == entities["program_id"]]
            if len(cands) == 1:
                sid = cands[0]["id"]
            else:
    ...
    if intent == "send_reminders":
        out = create_reminder_approval(entities.get("program_id"))
